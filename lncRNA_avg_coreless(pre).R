library(openxlsx)
setwd("/Users/uemurakohei/Desktop/ncRNA/lncRNA/16_categories_coreless")
 ...
 #
  ...
  for(DHS.Support in unique(lncRNA_id[,"DHS.Support"])){
    ...
    for(gene_class in unique(lncRNA_id[,"Gene.Class"])){
      ...
      # core less #
      lncRNA_id.i <- intersect(sapply(gene_id_coreless, function(x) unlist(strsplit(x ,"\\|"))[1]), lncRNA_id.i)
      ...
      if(count.i.j == 0){
        ...
        pdf(sprintf("%s_%s_average.pdf", property_name, DHS.Support),width=7.5, height=3)
